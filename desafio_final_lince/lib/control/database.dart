...
class DatabaseHelper {
  ...
  Future<List<Vacancy>> getVacancies() async {
    ...
    var groceries = await db.query('vv', orderBy: 'dateTime desc');
  ...
  Future<int> removeList(int id, String exitDate, double valor) async {
    final db = await instance.database;
    return db.update('vv', {'valor': valor, 'parked': 0, 'exitData': exitDate},
  ...
  /// List car
  Future<List<Vacancy>> listCar() async {
    var db = await instance.database;
    var groceries = await db.query('vv', orderBy: 'parked desc', groupBy: 'id');
    final groceryList = groceries.isNotEmpty
        ? groceries.map(Vacancy.fromMap).toList()
        : <Vacancy>[];
    return groceryList;
  ...
  /// Yield vacancies on day
  Future<List<Income>> getPrices() async {
    var db = await instance.database;
    final rows = await db.query('vv',
        columns: [
          'exitData',
          'sum(valor)',
        ],where: 'parked == ?',whereArgs: ['0'],
        groupBy: "strftime('%d-%m-%Y', exitData)");

    final list = <Income>[];
    for (final row in rows) {
      list.add(Income.yiedl(row));
    }
    return list;
  }
}
...
class Income {
  ...
  double? valueYield;
  /// Income constructor
  Income(this.dayYield, this.valueYield);

  /// Method yiedl
  Income.yiedl(Map<String, dynamic> json)
      : dayYield = json['exitData'],
        valueYield = json['sum(valor)'];
}
